...
%                   [p(3,1), p(3,1) + pChange3(3,1)], [p(3,2), p(3,2) + pChange3(3,2)], [p(3,3), p(3,3)+pChange3(3,3)], '-o');
            ...
            % Making a vector into the plot.
            vectorPos = [p(4, 1), p(4, 2), p(4, 3)];
            measuredVel = self.robot.measured_js(true, true);
            velVector = self.robot.fdk3001(measuredVel(1, :), measuredVel(2, :));
            quiver3(p(4, 1), p(4, 2), p(4, 3), velVector(1,1)*.2, velVector(2,1)*.2, velVector(3,1)*.2, '-o');
        ...
        % Draws the stickmodel, makes a trajectory
        function drawAndTrajectory(self, jointVal, trajVal)
