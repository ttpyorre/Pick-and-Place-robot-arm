    ...
    methods
        ...
        function PV = run_trajectory(self, trajCoeff, time, inJointSpace)
            ...
            while time - timeOff > 0
                ...
                jp = self.jacob3001(input);
                posVelData = self.measured_js(true,true);
                if inJointSpace
                    % Servo takes the data in as angles, we need to convert
                    % position to make the arm go to the right position.                    
                    posData = [posData; posVelData(1,:)]; 
                    velData = [velData; posVelData(2,:)];
                   
                    %input = self.ik3001(input);
                    %for velocity kinematics only
                    % Data as position, use the below functions.
                    %fk = self.fk3001(posVelData(1,:)');
                    %posData = [posData; transpose(fk(1:3,4))];
                else 
                    % Forward kinematics needed for position data.
                    input = self.ik3001(input);
                    fk = self.fk3001(posVelData(1,:)');
                    
                    %LAB 4 ONLY: record determinant data
                    posData = [posData; fk(1:3,4)', det(jp(1:3,:)), timeOff];
                    
                    %NORMAL:
                    %posData = [posData; fk(1:3,4)'];
                    
                    %LAB 4 ONLY: trace the motion of robot arm as it movest
                    %towards the singluarity configuration
                    
                    writematrix(posData, 'singluarityMotion.csv');
                end
                timeData = [timeData; timeOff];
                velocityKinematics = [velocityKinematics; transpose(self.fdk3001(input,posVelData(2,:)))];
                self.servo_jp(input);
                % Here we draw the stick model.
                % Another option is to have this as an interrupt, so we run
                % it slower than the trajectory, so the trajectory will be
                % smooth.
                %self.model.FKdraw(input');
                %fdk = self.fdk3001(posVelData(1, 1:3), posVelData(2, 1:3));
                
                pause(0.1);
                timeOff = toc;
            end
            % Assigns the data to the matrix we return, based on if were in
            % Joint space or not
            if inJointSpace
                %for velocity kinematics only
                PV(:,1:6) = velocityKinematics;

%                 PV(:,1:3) = posData;
%                 PV(:,4:6) = velData;
                PV(:,7) = timeData; %Again, if we want degrees.
                %PV(:,4) = timeData;
                ...
                %LAB 4 ONLY: return position data, determinant data, time
                PV(:,1:5) = posData;
        ...
        %takes a 1x3 vector of goal positions and a velocity value in mm/s
        function iv = idk3001(self,currentPosition,goalPosition,goalVelocity)
             %unit vector

            unitVector = (goalPosition - currentPosition) ./ norm(goalPosition - currentPosition);
            
            %mult by velocity
            velocityVector = goalVelocity.*unitVector;
            
            %inverse velocity
            jointAngles = self.ik3001(currentPosition);
            J = self.jacob3001(jointAngles);
            jUp = J(1:3,:);
            iv = velocityVector*jUp;
            return
        end
        ...
        % is within that error 
        %returns false otherwise
        function goal = atGivenGoalPos(self,errorDeg,goalPos)
